...
class BetterPlayerController {
  ...
  Future<void> setupDataSource(BetterPlayerDataSource betterPlayerDataSource) async {
    ...
    if (videoPlayerController == null) {
      ...
      // PATCH: turn the notification and lock-screen skip taps into ordinary
      // player events, so a host app can listen for them the same way it
      // listens for play, pause and the rest.
      videoPlayerController?.onSkipToNext =
          () => _postEvent(BetterPlayerEvent(BetterPlayerEventType.skipToNext));
      videoPlayerController?.onSkipToPrevious = () =>
          _postEvent(BetterPlayerEvent(BetterPlayerEventType.skipToPrevious));
    }
