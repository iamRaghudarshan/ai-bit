...
class VideoPlayerController extends ValueNotifier<VideoPlayerValue> {
  ...
  /// PATCH: notification and lock-screen skip buttons.
  ///
  /// These are one-off signals rather than state, so they cannot ride on
  /// [value] the way everything else here does. The host app owns the queue
  /// and decides what "next" means, so they are surfaced as callbacks.
  ...
  Future<void> _create() async {
    ...
    void eventListener(VideoEvent event) {
      ...
      switch (event.eventType) {
        ...
        case VideoEventType.skipToNext:
          onSkipToNext?.call();
        case VideoEventType.skipToPrevious:
